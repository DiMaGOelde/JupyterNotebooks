...
def import_data(tree, node_nr, data):
    ...
    if tree.tree_nodes[node_nr].node_type == 'leaf':
        tree.tree_nodes[node_nr].subset = data

    else:
        tree.tree_nodes[node_nr].subset = data
        children = tree.tree_nodes[node_nr].child_nodes
        attribute = tree.tree_nodes[node_nr].node_split.split_attribute
        
        if tree.tree_nodes[node_nr].node_split.split_type == 'numerical':
            bound = tree.tree_nodes[node_nr].node_split.split_bound
            ...
            split_values = tree.tree_nodes[node_nr].node_split.split_values
...
class DecisionTree:
    ...
    def prediction_data(self, data, node_nr = 1, list_subsets = []):
        ...
        if self.tree_nodes[node_nr].node_type == 'leaf':
            current_data['Prediction'] = self.tree_nodes[node_nr].node_label
            ...
            next_split = self.tree_nodes[node_nr].node_split
            if next_split.split_type == 'numerical':
                next_data1 = current_data[current_data[next_split.split_attribute]<=next_split.split_bound] 
                next_data2 = current_data[current_data[next_split.split_attribute]>next_split.split_bound] 
                ...
                for i in range(len(children)):
                    next_data = current_data[current_data[next_split.split_attribute] == next_split.split_values[i]] 
    ...
    def export(self, filename):
        ...
        for n in self.tree_nodes.values():
            ...
            if not n.node_split==None:
                for i in n.node_split.split_values:
                    split_values += str(i)
                    split_values += ','

                split_values = split_values[:-1]
                split_info = n.node_split.split_attribute + ';' + split_values + ';' + n.node_split.split_type + ';' + str(n.node_split.split_bound)
            else:
                split_info = str(None)

            node_info = str(n.node_nr) + ';' + n.node_label + ';' + n.node_type + ';' + split_info
